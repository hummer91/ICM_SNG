...
function renderColorDemo() {
    ...
    colorCategories.forEach(({ name, category }) => {
        ...
        Object.entries(colors).forEach(([key, value]) => {
            ...
            swatch.style.backgroundColor = typeof value === 'object' ? value['500'] || Object.values(value)[0] : value;
...
function renderButtonDemo() {
    ...
    variantRow.innerHTML = '<div style="width: 100%; margin-bottom: 16px;"><strong>버튼 변형</strong></div>';
    
    variants.forEach(variant => {
        variantRow.appendChild(Button.create({
    ...
    sizeRow.innerHTML = '<div style="width: 100%; margin-bottom: 16px;"><strong>버튼 크기</strong></div>';
    
    sizes.forEach(size => {
        sizeRow.appendChild(Button.create({
    ...
    stateRow.innerHTML = '<div style="width: 100%; margin-bottom: 16px;"><strong>버튼 상태</strong></div>';
    
    stateRow.appendChild(Button.create({
        text: '일반 버튼',
    ...
    groupDemo.appendChild(Button.createGroup([
...
function renderInputDemo() {
    ...
    // 기본 입력
    container.appendChild(Input.create({
        label: '이름',
    ...
    container.appendChild(Button.create({
        ...
        onClick: () => {
            const modal = Modal.create({
                title: '기본 모달',
                content: '<p>이것은 기본 모달 대화상자입니다.</p><p>다양한 내용을 포함할 수 있습니다.</p>',
                footer: [
                    { text: '닫기', variant: 'secondary', onClick: (e, modal) => modal.close() },
                    { text: '확인', variant: 'primary', onClick: (e, modal) => {
