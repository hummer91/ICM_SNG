...
- [ ] CSS 변수로 디자인 토큰 설정
  ...
  ```
- [ ] 타이포그래피 시스템
  - [ ] Inter Variable 폰트 통합
  - [ ] 폰트 크기 스케일 (12px-72px)
  - [ ] 폰트 굵기 (300-680)
- [ ] 스페이싱 시스템 (4px 베이스)
- [ ] 컴포넌트 스타일 가이드
  - [ ] 버튼 (primary, secondary, ghost, outline)
  - [ ] 입력 필드 (포커스 효과 포함)
  - [ ] 카드 컴포넌트
  - [ ] 모달 오버레이
- [ ] 애니메이션 설정
  - [ ] duration: 150ms, 200ms, 300ms
  - [ ] easing: cubic-bezier 함수들
- [ ] 반응형 브레이크포인트
...
- [ ] 신뢰할 수 있는 Nash Equilibrium 데이터 소스 확보
  - [ ] PokerCruncher, HoldemResources 등 검증된 소스 조사
  - [ ] 데이터 라이선스 확인 및 사용 권한 검토
  - [ ] 데이터 정확성 교차 검증
- [ ] ranges.json 데이터 파일 생성 (검증된 데이터 기반)
- [ ] 포지션별 Push 범위 계산
- [ ] 스택 깊이에 따른 범위 조정 로직
